...
let learningData = {
  b52: {
    ...
    multiTimeframeAnalysis: {
      shortTerm: { timeframe: 5, accuracy: 0 },
      mediumTerm: { timeframe: 15, accuracy: 0 },
      longTerm: { timeframe: 30, accuracy: 0 }
    },
    patternReliability: {},
...
function detectPatternMultiTimeframe(results, type) {
  const shortTerm = results.slice(0, 5);
  const mediumTerm = results.slice(0, 15);
  const longTerm = results.slice(0, 30);
  
  const analysis = {
    shortTerm: analyzeTimeframeWindow(shortTerm),
    mediumTerm: analyzeTimeframeWindow(mediumTerm),
    longTerm: analyzeTimeframeWindow(longTerm)
  };
  
  // Detect regime changes
  const regimes = detectMarketRegime(analysis);
  
  const weight = getPatternWeight(type, 'advanced_mtf');
  
  // Multi-timeframe confirmation
  const confirmations = [];
  if (analysis.shortTerm.dominant === analysis.mediumTerm.dominant) {
    confirmations.push('ST-MT');
  }
  if (analysis.mediumTerm.dominant === analysis.longTerm.dominant) {
    confirmations.push('MT-LT');
  }
  if (analysis.shortTerm.dominant === analysis.longTerm.dominant) {
    confirmations.push('ST-LT');
  }
  
  const confirmationScore = confirmations.length;
  
  if (confirmationScore >= 2 || regimes.isChanging) {
    let prediction;
    let confidence;
    
    if (confirmationScore >= 2) {
      prediction = analysis.shortTerm.dominant;
      confidence = Math.round((10 + confirmationScore * 5) * weight);
    } else {
      prediction = regimes.targetRegime;
    ...
    return {
      ...
      prediction,
      confidence,
      name: `MTF ${regimes.description} (${confirmations.join(', ')})`,
      patternId: 'advanced_mtf',
      analysis
    };
  }
  ...
  // Detect divergence
  if (analysis.shortTerm.dominant !== analysis.mediumTerm.dominant && 
      analysis.longTerm.consistency > 0.6) {
    return {
      detected: true,
      prediction: analysis.longTerm.dominant,
      confidence: Math.round(11 * weight),
      name: `MTF Divergence → LT ${analysis.longTerm.dominant}`,
      patternId: 'advanced_mtf',
      analysis
    };
  }
...
function analyzeTimeframeWindow(window) {
  if (window.length === 0) return { dominant: null, consistency: 0, ratio: 0 };
  ...
  return {
    ...
    consistency: Math.max(taiCount, xiuCount) / window.length,
    ratio: taiCount / window.length,
    strength: Math.abs(taiCount - xiuCount) / window.length
  };
}

function detectMarketRegime(analysis) {
  const stStrength = analysis.shortTerm.strength;
  const mtStrength = analysis.mediumTerm.strength;
  const ltStrength = analysis.longTerm.strength;
  
  const avgStrength = (stStrength + mtStrength + ltStrength) / 3;
  
  // Detect regime change
  const isChanging = (mtStrength > 0.4 && stStrength < 0.2) || 
                     (stStrength > 0.4 && mtStrength < 0.1);
  
  let description = 'Neutral';
  let targetRegime = analysis.mediumTerm.dominant;
  
  if (avgStrength > 0.5) {
    description = 'Strong Trend';
    targetRegime = analysis.longTerm.dominant;
  } else if (avgStrength > 0.3) {
    description = 'Moderate Trend';
    targetRegime = analysis.mediumTerm.dominant;
  } else if (isChanging) {
    description = 'Regime Change';
    targetRegime = analysis.shortTerm.dominant;
  }
...
function detectComplexCauPattern(results, type) {
  if (results.length < 8) return { detected: false };
  
  // Enhanced pattern detection with fuzzy matching
  const patterns = [
    { name: 'cau_1221_complex', func: checkCau1221Complex, minLength: 8 },
    { name: 'cau_spiral', func: checkCauSpiral, minLength: 10 },
    { name: 'cau_diamond', func: checkCauDiamond, minLength: 12 },
    { name: 'cau_wedge', func: checkCauWedge, minLength: 14 },
    { name: 'cau_rectangle', func: checkCauRectangle, minLength: 12 },
    { name: 'cau_triangle', func: checkCauTriangle, minLength: 10 }
  ];
  
  const weight = getPatternWeight(type, 'cau_complex');
  
  for (const pattern of patterns) {
    if (results.length >= pattern.minLength) {
      const result = pattern.func(results);
      if (result.detected) {
        return {
          detected: true,
          prediction: result.prediction,
          confidence: Math.round(result.confidence * weight),
          name: `Cầu ${pattern.name.replace(/_/g, ' ')}`,
          patternId: 'cau_complex',
          analysis: result
        };
      }
    }
...
function checkCau1221Complex(results) {
  const window = results.slice(0, 8);
  const segments = [];
  
  for (let i = 0; i < window.length - 1; i++) {
    if (window[i] !== window[i + 1]) {
      segments.push({ type: window[i], length: i });
      break;
    }
  }
  
  // Fuzzy pattern matching with tolerance
  const pattern1 = window.slice(0, 4);
  const expected = [pattern1[0], pattern1[1], pattern1[1], pattern1[3]];
  
  let matches = 0;
  for (let i = 0; i < 4; i++) {
    if (pattern1[i] === expected[i]) matches++;
  }
...
function checkCauSpiral(results) {
  const window = results.slice(0, 10);
  
  // Detect expanding/contracting pattern
  const streaks = [];
  let currentStreak = 1;
  ...
  // Spiral pattern: alternating increasing lengths
  if (streaks.length >= 4) {
    const increasing = streaks[2] > streaks[0] && streaks[3] > streaks[1];
    const decreasing = streaks[2] < streaks[0] && streaks[3] < streaks[1];
    
    if (increasing || decreasing) {
      const prediction = increasing ? 
        (window[0] === 'Tài' ? 'Xỉu' : 'Tài') : 
        window[0];
...
function checkCauDiamond(results) {
  if (results.length < 12) return { detected: false };
  
  // Diamond pattern: symmetric around center
  const window = results.slice(0, 12);
  const center = 5;
  
  let symmetricCount = 0;
  for (let i = 0; i < center; i++) {
    const left = window[center - i - 1];
    const right = window[center + i + 1];
    if (left !== right) symmetricCount++;
  }
  
  if (symmetricCount >= 4) {
    return {
      detected: true,
      prediction: window[0] === 'Tài' ? 'Xỉu' : 'Tài',
      confidence: 12,
      pattern: 'diamond',
      symmetry: symmetricCount / center
    };
  }
...
function checkCauWedge(results) {
  if (results.length < 14) return { detected: false };
  
  const window = results.slice(0, 14);
  
  // Calculate narrowing pattern
  const half1 = window.slice(0, 7);
  const half2 = window.slice(7, 14);
  
  const variance1 = calculateVariance(half1);
  const variance2 = calculateVariance(half2);
  
  if (variance2 < variance1 * 0.5) {
    return {
      detected: true,
      prediction: half2[half2.length - 1],
      confidence: 11,
      pattern: 'wedge',
      varianceReduction: variance1 / variance2
    };
  }
...
function calculateVariance(arr) {
  const moments = arr.map(r => r === 'Tài' ? 1 : 0);
  const mean = moments.reduce((a, b) => a + b, 0) / moments.length;
  return moments.reduce((a, b) => a + Math.pow(b - mean, 2), 0) / moments.length;
}

function checkCauRectangle(results) {
  if (results.length < 12) return { detected: false };
  
  const window = results.slice(0, 12);
  
  // Rectangle: alternating between two values
  const uniquePatterns = new Set(window.slice(0, 4).join(''));
  const repeated = [];
  
  for (let i = 2; i <= 4; i++) {
    const pattern1 = window.slice(0, i).join('');
    let count = 0;
    
    for (let j = 0; j < window.length - i + 1; j += i) {
      if (window.slice(j, j + i).join('') === pattern1) {
        count++;
      }
    }
...
function checkCauTriangle(results) {
  if (results.length < 10) return { detected: false };
  
  const window = results.slice(0, 10);
  
  // Triangle: alternating 1-2-3 pattern
  ...
  if (triangleMatches >= 3) {
    return {
      detected: true,
      prediction: window[0] === 'Tài' ? 'Xỉu' : 'Tài',
      confidence: 10 + triangleMatches,
      pattern: 'triangle',
      triangleMatches
    };
  }
...
// Enhanced Markov Chain with memory depth
function enhancedMarkovChain(results, data, type, depth = 3) {
  if (results.length < 20) return { detected: false };
  
  // Build higher-order transition matrix
  const transitions = {};
  
  for (let i = depth; i < results.length; i++) {
    const state = results.slice(i - depth, i).join(',');
    const next = results[i];
    
    if (!transitions[state]) {
      transitions[state] = { 'Tài': 0, 'Xỉu': 0 };
    }
    transitions[state][next]++;
  }
  
  const currentState = results.slice(0, depth).join(',');
  
  if (transitions[currentState]) {
    const total = transitions[currentState]['Tài'] + transitions[currentState]['Xỉu'];
    
    if (total > 3) {
      const taiProb = transitions[currentState]['Tài'] / total;
      
      const weight = getPatternWeight(type, 'markov_chain');
      const prediction = taiProb > 0.6 ? 'Tài' : (taiProb < 0.4 ? 'Xỉu' : null);
      
      if (prediction) {
        return {
          detected: true,
          prediction,
          confidence: Math.round(Math.min(15, Math.abs(taiProb - 0.5) * 30 + 8) * weight),
          name: `Markov Depth-${depth} (${currentState} → ${prediction}: ${(taiProb * 100).toFixed(0)}%)`,
          patternId: 'markov_chain',
          analysis: { transitions, currentState, depth, taiProb }
        };
      }
    }
  }
  ...
  if (depth > 1) {
    return enhancedMarkovChain(results, data, type, depth - 1);
  }
  
  return { detected: false };
}

// Enhanced Dice Pattern Analysis
function enhancedDiceAnalysis(data, type) {
  if (data.length < 5) return { detected: false };
  
  const recent = data.slice(0, 10);
  const dices = recent.map(d => [d.xuc_xac_1, d.xuc_xac_2, d.xuc_xac_3]);
  
  // Analyze dice combinations
  const combos = dices.map(d => d.sort().join('-'));
  ...
  const maxFreq = Math.max(...Object.values(comboFrequency));
  const mostFreqCombo = Object.keys(comboFrequency).find(k => comboFrequency[k] === maxFreq);
  
  const weight = getPatternWeight(type, 'dice_trend_line');
  
  // Detect repeating combinations
  if (maxFreq >= 3 && recent.length >= 5) {
    const comboSum = mostFreqCombo.split('-').reduce((a, b) => parseInt(a) + parseInt(b), 0);
    const prediction = comboSum >= 11 ? 'Xỉu' : 'Tài';
    
    return {
      detected: true,
      prediction,
      confidence: Math.round((8 + maxFreq * 2) * weight),
      name: `Dice Pattern: Combo ${mostFreqCombo} (${maxFreq}x) → ${prediction}`,
      patternId: 'dice_trend_line',
      analysis: { comboFrequency, mostFreqCombo, maxFreq }
    };
  }
  
  // Dice sum progression analysis
  const sums = recent.map(d => d.tong);
  const sumDiffs = [];
  for (let i = 1; i < sums.length; i++) {
    sumDiffs.push(sums[i] - sums[i - 1]);
  }
  
  const positiveDiffs = sumDiffs.filter(d => d > 0).length;
  const negativeDiffs = sumDiffs.filter(d => d < 0).length;
  
  if (Math.abs(positiveDiffs - negativeDiffs) >= 4) {
    const trend = positiveDiffs > negativeDiffs ? 'tăng' : 'giảm';
    const prediction = trend === 'tăng' ? 'Xỉu' : 'Tài';
    
    return {
      detected: true,
      prediction,
      confidence: Math.round(10 * weight),
      name: `Dice Trend: ${positiveDiffs}↑ ${negativeDiffs}↓ → ${prediction}`,
      patternId: 'dice_trend_line',
      analysis: { positiveDiffs, negativeDiffs, sumDiffs }
    };
  }
...
// Enhanced Confidence Calculation (No Random)
function calculateEnhancedConfidence(predictions, type, analysis) {
  let baseConfidence = 50;
  
  // Calculate confidence based on pattern agreement
  const topPredictions = predictions.slice(0, 5);
  const finalPrediction = topPredictions[0].prediction;
  
  // Pattern agreement score
  const agreementCount = topPredictions.filter(p => p.prediction === finalPrediction).length;
  const agreementBonus = agreementCount >= 4 ? 15 : (agreementCount >= 3 ? 10 : (agreementCount >= 2 ? 5 : 0));
  
  // Pattern strength score
  const avgConfidence = topPredictions.reduce((s, p) => s + p.confidence, 0) / topPredictions.length;
  const strengthBonus = Math.min(10, avgConfidence / 2);
  
  // Pattern priority score
  const priorityBonus = topPredictions[0].priority * 2;
  
  // Historical accuracy bonus
  const patternAccuracy = getPatternHistoryAccuracy(type, topPredictions[0].patternId);
  const accuracyBonus = patternAccuracy > 50 ? Math.round((patternAccuracy - 50) / 5) : 0;
  
  // Market regime bonus
  const regimeBonus = analysis.marketRegime === 'neutral' ? 0 : 
                      (analysis.marketRegime === 'Strong Trend' ? 8 : 4);
  
  // Timeframe confirmation bonus
  const mtfBonus = (analysis.mtfConfirmation || 0) * 5;
  ...
  finalConfidence = applyPatternAdjustments(finalConfidence, topPredictions, type);
  
  // Adaptive boost based on recent performance
  const adaptiveBoost = getAdaptiveConfidenceBoost(type);
  finalConfidence += adaptiveBoost;
  
  // Cap confidence between 45 and 90
  finalConfidence = Math.max(45, Math.min(90, Math.round(finalConfidence)));
  
  return finalConfidence;
}

function getPatternHistoryAccuracy(type, patternId) {
  if (!patternId || !learningData[type].patternStats[patternId]) {
    return 50;
  }
...
function applyPatternAdjustments(confidence, predictions, type) {
  // Adjust based on pattern diversity
  const patternTypes = new Set(predictions.map(p => p.patternId));
  const diversityBonus = Math.min(5, patternTypes.size * 1.5);
  
  // Adjust for pattern harmony
  const predictionsAgreement = predictions.filter(p => p.prediction === predictions[0].prediction).length;
  const harmonyFactor = predictionsAgreement / predictions.length;
  
  let adjusted = confidence;
  
  if (harmonyFactor > 0.8 && predictions.length >= 3) {
    adjusted += 3;
  }
  
  if (predictions.length >= 5 && predictionsAgreement >= 4) {
    adjusted += 2;
  }
...
// ==================== MODIFIED MAIN CALCULATION FUNCTION ====================
...
function calculateAdvancedPrediction(data, type) {
  ...
  const mtfAnalysis = detectPatternMultiTimeframe(results, type);
  if (mtfAnalysis.detected) {
    predictions.push({ 
      prediction: mtfAnalysis.prediction, 
      confidence: mtfAnalysis.confidence, 
      priority: 15, 
      name: mtfAnalysis.name,
      patternId: 'advanced_mtf'
    });
    factors.push(mtfAnalysis.name);
    allPatterns.push(mtfAnalysis);
  }
  
  const complexCau = detectComplexCauPattern(results, type);
  if (complexCau.detected) {
    predictions.push({ 
      prediction: complexCau.prediction, 
      confidence: complexCau.confidence, 
      priority: 14, 
      name: complexCau.name,
      patternId: 'cau_complex'
    });
    factors.push(complexCau.name);
    allPatterns.push(complexCau);
  }
  
  const enhancedMarkov = enhancedMarkovChain(results, data, type);
  if (enhancedMarkov.detected) {
    predictions.push({ 
      prediction: enhancedMarkov.prediction, 
      confidence: enhancedMarkov.confidence, 
      priority: 12, 
      name: enhancedMarkov.name,
      patternId: 'markov_chain'
    });
    factors.push(enhancedMarkov.name);
    allPatterns.push(enhancedMarkov);
  }
  
  const enhancedDice = enhancedDiceAnalysis(data, type);
  if (enhancedDice.detected) {
    predictions.push({ 
      prediction: enhancedDice.prediction, 
      confidence: enhancedDice.confidence, 
      priority: 13, 
      name: enhancedDice.name,
      patternId: 'dice_trend_line'
    });
    factors.push(enhancedDice.name);
    allPatterns.push(enhancedDice);
  }
  
  // Original pattern detections (keep all existing patterns)
  const cauBet = analyzeCauBet(results, type);
  if (cauBet.detected) {
    predictions.push({ prediction: cauBet.prediction, confidence: cauBet.confidence, priority: 10, name: cauBet.name, patternId: 'cau_bet' });
  ...
  if (cauDao11.detected) {
    predictions.push({ prediction: cauDao11.prediction, confidence: cauDao11.confidence, priority: 9, name: cauDao11.name, patternId: 'cau_dao_11' });
  ...
  if (cau22.detected) {
    predictions.push({ prediction: cau22.prediction, confidence: cau22.confidence, priority: 8, name: cau22.name, patternId: 'cau_22' });
  ...
  if (cau33.detected) {
    predictions.push({ prediction: cau33.prediction, confidence: cau33.confidence, priority: 8, name: cau33.name, patternId: 'cau_33' });
  ...
  if (cau121.detected) {
    predictions.push({ prediction: cau121.prediction, confidence: cau121.confidence, priority: 7, name: cau121.name, patternId: 'cau_121' });
  ...
  if (cau123.detected) {
    predictions.push({ prediction: cau123.prediction, confidence: cau123.confidence, priority: 7, name: cau123.name, patternId: 'cau_123' });
  ...
  if (cau321.detected) {
    predictions.push({ prediction: cau321.prediction, confidence: cau321.confidence, priority: 7, name: cau321.name, patternId: 'cau_321' });
  ...
  if (cauNhayCoc.detected) {
    predictions.push({ prediction: cauNhayCoc.prediction, confidence: cauNhayCoc.confidence, priority: 6, name: cauNhayCoc.name, patternId: 'cau_nhay_coc' });
  ...
  if (cauNhipNghieng.detected) {
    predictions.push({ prediction: cauNhipNghieng.prediction, confidence: cauNhipNghieng.confidence, priority: 7, name: cauNhipNghieng.name, patternId: 'cau_nhip_nghieng' });
  ...
  if (cau3Van1.detected) {
    predictions.push({ prediction: cau3Van1.prediction, confidence: cau3Van1.confidence, priority: 6, name: cau3Van1.name, patternId: 'cau_3van1' });
  ...
  if (cauBeCau.detected) {
    predictions.push({ prediction: cauBeCau.prediction, confidence: cauBeCau.confidence, priority: 8, name: cauBeCau.name, patternId: 'cau_be_cau' });
  ...
  if (cyclePattern.detected) {
    predictions.push({ prediction: cyclePattern.prediction, confidence: cyclePattern.confidence, priority: 7, name: cyclePattern.name, patternId: 'cau_chu_ky' });
  ...
  if (cau44.detected) {
    predictions.push({ prediction: cau44.prediction, confidence: cau44.confidence, priority: 9, name: cau44.name, patternId: 'cau_44' });
  ...
  if (cau55.detected) {
    predictions.push({ prediction: cau55.prediction, confidence: cau55.confidence, priority: 9, name: cau55.name, patternId: 'cau_55' });
  ...
  if (cau212.detected) {
    predictions.push({ prediction: cau212.prediction, confidence: cau212.confidence, priority: 8, name: cau212.name, patternId: 'cau_212' });
  ...
  if (cau1221.detected) {
    predictions.push({ prediction: cau1221.prediction, confidence: cau1221.confidence, priority: 8, name: cau1221.name, patternId: 'cau_1221' });
  ...
  if (cau2112.detected) {
    predictions.push({ prediction: cau2112.prediction, confidence: cau2112.confidence, priority: 8, name: cau2112.name, patternId: 'cau_2112' });
  ...
  if (cauGap.detected) {
    predictions.push({ prediction: cauGap.prediction, confidence: cauGap.confidence, priority: 7, name: cauGap.name, patternId: 'cau_gap' });
  ...
  if (cauZiczac.detected) {
    predictions.push({ prediction: cauZiczac.prediction, confidence: cauZiczac.confidence, priority: 8, name: cauZiczac.name, patternId: 'cau_ziczac' });
  ...
  if (cauDoi.detected) {
    predictions.push({ prediction: cauDoi.prediction, confidence: cauDoi.confidence, priority: 8, name: cauDoi.name, patternId: 'cau_doi' });
  ...
  if (cauRong.detected) {
    predictions.push({ prediction: cauRong.prediction, confidence: cauRong.confidence, priority: 10, name: cauRong.name, patternId: 'cau_rong' });
  ...
  if (smartBet.detected) {
    predictions.push({ prediction: smartBet.prediction, confidence: smartBet.confidence, priority: 9, name: smartBet.name, patternId: 'smart_bet' });
  ...
  if (diceTrendLine.detected) {
    predictions.push({ prediction: diceTrendLine.prediction, confidence: diceTrendLine.confidence, priority: 11, name: diceTrendLine.name, patternId: 'dice_trend_line' });
  ...
  if (breakPattern.detected) {
    predictions.push({ prediction: breakPattern.prediction, confidence: breakPattern.confidence, priority: 12, name: breakPattern.name, patternId: 'break_pattern' });
  ...
  if (dayGay.detected) {
    predictions.push({ prediction: dayGay.prediction, confidence: dayGay.confidence, priority: 13, name: dayGay.name, patternId: 'day_gay' });
  ...
  if (distribution.imbalance > 0.2) {
    ...
    predictions.push({ prediction: minority, confidence: Math.round(6 * weight), priority: 5, name: 'Phân bố lệch', patternId: 'distribution' });
  ...
  if (dicePatterns.averageSum > 11.5) {
    ...
    predictions.push({ prediction: 'Xỉu', confidence: Math.round(5 * weight), priority: 4, name: 'Tổng TB cao', patternId: 'dice_pattern' });
    factors.push(`Tổng TB cao (${dicePatterns.averageSum.toFixed(1)})`);
  } else if (dicePatterns.averageSum < 9.5) {
    const weight = getPatternWeight(type, 'dice_pattern');
    predictions.push({ prediction: 'Tài', confidence: Math.round(5 * weight), priority: 4, name: 'Tổng TB thấp', patternId: 'dice_pattern' });
  ...
  if (sumTrend.strength > 0.4) {
    ...
    predictions.push({ prediction: trendPrediction, confidence: Math.round(4 * weight), priority: 3, name: 'Xu hướng tổng', patternId: 'sum_trend' });
  ...
  if (edgeCases.detected) {
    predictions.push({ prediction: edgeCases.prediction, confidence: edgeCases.confidence, priority: 5, name: edgeCases.name, patternId: 'edge_cases' });
  ...
  if (momentum.window_3 && momentum.window_10) {
    ...
    if (shortTermDiff > 0.3) {
      ...
      predictions.push({ prediction: reversePrediction, confidence: Math.round(5 * weight), priority: 4, name: 'Biến động ngắn hạn', patternId: 'momentum' });
  ...
  if (fibonacciPattern.detected) {
    predictions.push({ prediction: fibonacciPattern.prediction, confidence: fibonacciPattern.confidence, priority: 8, name: fibonacciPattern.name, patternId: 'fibonacci' });
  ...
  if (momentumPattern.detected) {
    predictions.push({ prediction: momentumPattern.prediction, confidence: momentumPattern.confidence, priority: 9, name: momentumPattern.name, patternId: 'momentum' });
  ...
  if (resistanceSupport.detected) {
    predictions.push({ prediction: resistanceSupport.prediction, confidence: resistanceSupport.confidence, priority: 10, name: resistanceSupport.name, patternId: 'resistance_support' });
  ...
  if (wavePattern.detected) {
    predictions.push({ prediction: wavePattern.prediction, confidence: wavePattern.confidence, priority: 8, name: wavePattern.name, patternId: 'wave' });
  ...
  if (goldenRatio.detected) {
    predictions.push({ prediction: goldenRatio.prediction, confidence: goldenRatio.confidence, priority: 9, name: goldenRatio.name, patternId: 'golden_ratio' });
    factors.push(goldenRatio.name);
    allPatterns.push(goldenRatio);
  }
  
  const markovChain = analyzeMarkovChain(results, last50, type);
  if (markovChain.detected && !enhancedMarkov.detected) {
    predictions.push({ prediction: markovChain.prediction, confidence: markovChain.confidence, priority: 12, name: markovChain.name, patternId: 'markov_chain' });
  ...
  if (movingAvgDrift.detected) {
    predictions.push({ prediction: movingAvgDrift.prediction, confidence: movingAvgDrift.confidence, priority: 11, name: movingAvgDrift.name, patternId: 'moving_avg_drift' });
  ...
  if (sumPressure.detected) {
    predictions.push({ prediction: sumPressure.prediction, confidence: sumPressure.confidence, priority: 11, name: sumPressure.name, patternId: 'sum_pressure' });
  ...
  if (volatilityPattern.detected) {
    predictions.push({ prediction: volatilityPattern.prediction, confidence: volatilityPattern.confidence, priority: 10, name: volatilityPattern.name, patternId: 'volatility' });
  ...
  if (sunHotCold.detected) {
    predictions.push({ prediction: sunHotCold.prediction, confidence: sunHotCold.confidence, priority: 13, name: sunHotCold.name, patternId: 'sun_hot_cold' });
  ...
  if (sunStreakBreak.detected) {
    predictions.push({ prediction: sunStreakBreak.prediction, confidence: sunStreakBreak.confidence, priority: 14, name: sunStreakBreak.name, patternId: 'sun_streak_break' });
  ...
  if (sunBalance.detected) {
    predictions.push({ prediction: sunBalance.prediction, confidence: sunBalance.confidence, priority: 12, name: sunBalance.name, patternId: 'sun_balance' });
  ...
  if (sunMomentumShift.detected) {
    predictions.push({ prediction: sunMomentumShift.prediction, confidence: sunMomentumShift.confidence, priority: 13, name: sunMomentumShift.name, patternId: 'sun_momentum_shift' });
  ...
  if (predictions.length === 0) {
    ...
    predictions.push({ prediction: cauTuNhien.prediction, confidence: cauTuNhien.confidence, priority: 1, name: cauTuNhien.name, patternId: 'cau_tu_nhien' });
  ...
  // Enhanced confidence calculation (no random)
  const analysis = {
    marketRegime: learningData[type].marketRegime || 'neutral',
    mtfConfirmation: mtfAnalysis.detected ? 2 : 0
  };
  
  const finalConfidence = calculateEnhancedConfidence(predictions, type, analysis);
  ...
  return {
    ...
    detailedAnalysis: {
      ...
      mtfConfirmation: analysis.mtfConfirmation,
...
// Keep all existing pattern analysis functions (analyzeCauBet, analyzeCauDao11, etc.)
// These functions remain unchanged from the original code
  ...
  if (sameCount === 2 && upCount === 1) {
    ...
    if (sameDiceValues[0] === sameDiceValues[1]) {
      return {
        ...
        name: `Dây Gãy (2 dây thẳng + 1 lên → Xỉu)`,
        patternId: 'day_gay',
  ...
  if (sameCount === 2 && downCount === 1) {
    ...
    if (sameDiceValues[0] === sameDiceValues[1]) {
      return {
        ...
        prediction: 'Tài',
        confidence: Math.round(14 * weight),
        name: `Dây Gãy (2 dây thẳng + 1 xuống → Tài)`,
        patternId: 'day_gay',
        analysis: { upCount, downCount, sameCount, currentDices, previousDices, directions, sameDiceValues }
      };
    }
